...
repository root에서 다음 명령을 실행한다. 첫 번째 스크립트는 로컬 도구, Azure CLI 버전과 로그인만 확인한다. 두 번째 스크립트가 extension, provider, quota, 리소스 생성과 RBAC 역할 할당을 처리하고 분석, egress와 hash 검증 결과를 `.work/dynamic-sessions/python/`에 저장한다.
...
기본값은 현재 `az` subscription, `koreacentral`, `rg-ai-workspace-dynamic-sessions-lab`, `ai-workspace-python-sbx`다. 다른 값을 쓰려면 2절의 환경 변수를 명령 실행 전에 설정한다. quota가 없거나 역할을 할당할 권한이 부족하면 해당 Azure 명령에서 중단된다.
...
```bash
RUN_LIMIT_TESTS=yes bash scripts/dynamic-sessions/python-lab.sh
```
...
export LAB_WORK_DIR="$PWD/.work/dynamic-sessions/python-manual"
...
| 구성 요소 | 관리자 책임 |
| --- | --- |
| `dynamic_sessions/policy.py` | 위험 요청을 LLM과 session 할당 전에 분류 |
| `dynamic_sessions/python_session.py` | token, endpoint와 identifier를 backend에서 관리 |
...
```bash
cd "${REPO_ROOT:-$PWD}"
test -f scripts/dynamic-sessions/agent-lab.sh || {
...
LLM_PROVIDER=stub bash scripts/dynamic-sessions/agent-lab.sh

# 자동 스크립트 없이 동일한 backend entrypoint를 직접 실행할 때
LLM_PROVIDER=stub python -m dynamic_sessions.cli
```
