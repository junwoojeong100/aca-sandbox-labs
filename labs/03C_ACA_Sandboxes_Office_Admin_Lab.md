...
> **Dynamic Sessions Office(실습 2A)와의 핵심 차이**  
> Dynamic Sessions는 container 안에 HTTP 서버(`server.py`)를 두고 session pool management endpoint를 통해 호출한다.  
> ACA Sandboxes는 `sandbox.exec()`로 LibreOffice·Python 명령을 직접 실행하므로 **HTTP 서버 계층이 필요 없다**.
...
export LAB_WORK_DIR="$PWD/.work/sandboxes-office"
...
ACA Sandboxes에서 Office 작업은 **HTTP 서버가 필요 없다**. 이 repository의
기존 `office-container/` image에는 Dynamic Sessions용 HTTP 서버가 포함돼
있지만, 6절에서 disk image entrypoint를 `sleep infinity`로 덮어써 서버를
실행하지 않고 LibreOffice와 Python 도구만 재사용한다.

`office-container/` 디렉터리의 `Dockerfile`을 Sandboxes용으로 확인한다.  
실습 2A에서 이미 빌드한 이미지가 있다면 동일 이미지를 재사용할 수 있다.
...
az acr build \
  ...
  --file office-container/Dockerfile \
  office-container/ \
  --output none
...
이 단계는 Dynamic Sessions에 없는 과정으로, Sandboxes가 container image를 VM 루트 파일시스템으로 변환한다.
...
WORK_DIR        = os.environ.get("LAB_WORK_DIR", ".work/sandboxes-office")
...
> `LIBREOFFICE_MISSING`이 나오면 office-container Dockerfile을 확인하고 disk image를 재등록한다.

## 8. Health 및 도구 확인

Dynamic Sessions의 `/health` API에 대응하는 단계다.  
Sandboxes에서는 HTTP 서버 없이 `exec()`로 직접 도구를 확인한다.
...
Dynamic Sessions의 `/generate` REST API에 대응하는 단계다.  
Sandboxes에서는 Python 라이브러리와 LibreOffice를 `exec()`로 직접 호출한다.
...
```bash
file .work/sandboxes-office/report.docx .work/sandboxes-office/report.pdf \
     .work/sandboxes-office/slides.pptx .work/sandboxes-office/slides.pdf \
     .work/sandboxes-office/report.xlsx
head -c 4 .work/sandboxes-office/report.pdf
```

## 10. 허용 목록 기반 형식 변환

Dynamic Sessions의 `/convert` REST API에 대응한다.  
Sandboxes에서는 허용된 변환 matrix를 gateway가 관리하고, `exec()`로 LibreOffice를 호출한다.
...
Dynamic Sessions의 `/edit` REST API에 대응한다.  
...
## 부록: Dynamic Sessions Office(실습 2A)와 ACA Sandboxes Office(실습 3C) 비교

| 항목 | Dynamic Sessions Office (실습 2A) | ACA Sandboxes Office (실습 3C) |
| --- | --- | --- |
| **Container 역할** | HTTP 서버(`server.py`) 포함 | 도구만 설치, HTTP 서버 불필요 |
| **실행 방식** | `POST /generate`, `POST /convert`, `POST /edit` REST API | `sandbox.exec("python3 /work/generate.py")` |
