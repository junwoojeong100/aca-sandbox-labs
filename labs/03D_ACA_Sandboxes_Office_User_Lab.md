...
## Dynamic Sessions Office(실습 2B)와의 차이점

| 항목 | Dynamic Sessions (실습 2B) | ACA Sandboxes (실습 3D) |
| --- | --- | --- |
| **Backend 실행 방식** | `POST /generate`, `/convert`, `/edit` REST API | SDK `sandbox.exec()` |
| **파일 I/O** | `POST /files` / `GET /files/{name}/content` | SDK `write_file()` / `read_file()` |
| **Sandbox 식별** | session identifier (`?identifier=`) | Sandbox ID |
| **상태 보존** | cooldown 후 삭제 | suspend/resume |
| **HTTP 서버** | container 안에 필요 | 불필요 |

사용자 API의 요청·응답 형식은 실습 2B와 동일하게 유지한다.
...
```bash
mkdir -p .work/sandboxes-office-user
...
curl --fail-with-body --silent --show-error \
  ...
  --output .work/sandboxes-office-user/create.json

jq . .work/sandboxes-office-user/create.json
export DOCUMENT_JOB_ID=$(jq -r '.id' .work/sandboxes-office-user/create.json)
```
...
curl --fail-with-body --silent --show-error \
  ...
  --header "X-Demo-User: $DEMO_USER" \
  --output .work/sandboxes-office-user/report.pdf

file .work/sandboxes-office-user/report.pdf
```
...
curl --fail-with-body --silent --show-error \
  ...
  --header "Content-Type: application/json" \
  --data '{}' \
  --output .work/sandboxes-office-user/approved.json

jq '. | {status, files}' .work/sandboxes-office-user/approved.json
```
...
| backend 종류(`sandboxes`) | SandboxGroup 이름·Resource Group |
