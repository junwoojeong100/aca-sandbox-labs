...
- Workload profile을 사용하는 Container Apps Environment
- Log Analytics 연결
- Startup·Liveness probe 구성
- `EgressDisabled` 설정
- `/health` API 응답
...
> Custom Container pool은 ready session을 최소 1개 요구할 수 있다. Custom Container는 active·ready session을 수용하는 전용 E16 `nodeCount` 기반 Dedicated plan으로 과금된다. 이 실습은 Container Apps Environment, ACR, Log Analytics도 만들며 이들은 pool을 삭제해도 남는다. 비용 상세는 [Dynamic Sessions Reference Architecture](../../docs/dynamic-sessions/architecture.md#lifecycle-capacity-performance-및-비용)를 참고하고, 실습 후에는 17절의 정리 절차를 검토한다.
...
- `dynamic_sessions/office_image/`의 Dockerfile·probes.yaml과 공통 `office_gateway/container_api.py`
...
repository root에서 다음 명령을 실행하면 ACR, identity, workspace, environment, pool, 네 형식과 hash, session, logs와 metrics를 자동 검증한다. 결과는 `.work/dynamic-sessions/office/`에 저장한다.
...
export LAB_WORK_DIR="$PWD/.work/dynamic-sessions/office-manual"
...
- `provisioningState: Succeeded` 확인
- ACR admin user 비활성화
...
## 7. Container Apps Environment 생성
...
## 9. Office Custom Container Pool 생성
...
## 11. Health endpoint 확인

이후 생성되는 응답과 문서는 `.work/dynamic-sessions/office-manual/`에 저장한다.
...
| 증상 | 원인과 조치 |
| --- | --- |
| ACR build가 Dockerfile을 찾지 못함 | `--file "$PWD/dynamic_sessions/office_image/Dockerfile"`처럼 절대 경로 사용 |
...
```bash
cd "${REPO_ROOT:-$PWD}"
test -f scripts/dynamic-sessions/cleanup.sh || {
...
```bash
CONFIRM_DELETE=yes bash scripts/dynamic-sessions/cleanup.sh
```
...
- ACR Basic, admin user 비활성화
- User-assigned identity와 AcrPull
- Workload profile을 사용하는 Environment
- Log Analytics 연결
- Image build 성공
- Custom Container Pool 상태 `Succeeded`
- Ready session 1개
- Startup·Liveness probe 구성
- `/health` HTTP 200
- LibreOffice 7.4.7.2, Pandoc 2.17.1.1, Poppler 22.12.0
- OpenPyXL 3.1.5, python-pptx 1.0.2
- DOCX, PDF, PPTX, XLSX 생성 HTTP 200
...
| Clean rebuild와 Pool image update | 성공 |
...
| 항목 | 결과 |
| --- | --- |
| ACR cloud build 결과 | 성공, image `office-sandbox:20260727224541` |
| Image digest 값 | `sha256:719165f1725599562221736110d300c40cdaf2e3aa8d61dd6eb535e5d840ed2b` |
