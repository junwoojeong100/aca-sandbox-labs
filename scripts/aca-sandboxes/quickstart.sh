...
MODE=${1:-python}
VENV_DIR=${ACA_VENV_DIR:-"$REPO_ROOT/.work/aca-sandboxes/venv"}

case "$MODE" in
  python|office|all) ;;
  *) die "Usage: bash scripts/aca-sandboxes/quickstart.sh [python|office|all]" ;;
esac
...
case "$MODE" in
  python)
    "$VENV_DIR/bin/python" scripts/aca-sandboxes/python-lab.py
    ;;
  office)
    "$VENV_DIR/bin/python" scripts/aca-sandboxes/python-lab.py --provision-only
    "$VENV_DIR/bin/python" scripts/aca-sandboxes/office-lab.py
    ;;
  all)
    "$VENV_DIR/bin/python" scripts/aca-sandboxes/python-lab.py
    "$VENV_DIR/bin/python" scripts/aca-sandboxes/office-lab.py
