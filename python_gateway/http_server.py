"""Backend-neutral HTTP transport for Python analysis jobs."""
...
class Handler(BaseHTTPRequestHandler):
    ...
    def user(self) -> str:
        return self.service.validate_user(
            self.headers.get("X-Demo-User", "")
        )
    ...
    def read_multipart(
        ...
        if not content_type.lower().startswith("multipart/form-data"):
            raise GatewayError(
                415,
                "Content-Type은 multipart/form-data여야 한다",
            )
        try:
        ...
        for part in message.iter_parts():
            ...
            try:
                ...
                raise GatewayError(
                    400,
                    "multipart text field는 UTF-8이어야 한다",
                ) from error
    ...
    def do_POST(self) -> None:
        try:
            ...
            if parts == ["api", "analysis-jobs"]:
                request_text, attachments, expected_outputs = (
                    self.read_multipart()
                )
                self.send_json(
                    HTTPStatus.CREATED,
