...
[실습 1의 8.1절](01_Python_Code_Interpreter_Lab.md#81-사전-설치-라이브러리-확인)에서 확인했듯 **`python-docx`, `python-pptx`, `openpyxl`, `reportlab`은 Python pool에도 이미 설치돼 있다.**
...
Fast Path는 필요한 extension과 provider를 직접 준비하므로 실습 1이나 Python pool이 필요 없다. 수동 명령을 따라갈 때만 [실습 1의 CLI와 provider 준비](01_Python_Code_Interpreter_Lab.md#3-cli와-provider-준비)를 먼저 완료한다.
