...
python3 -c \
  'import ast, pathlib; ast.parse(pathlib.Path("office_gateway/container_api.py").read_text())'
...
for source in sorted(pathlib.Path(".").glob("**/*.py")):
    if ".work" in source.parts or ".git" in source.parts:
        continue
...
documents = (
    ...
    + list((root / "labs").rglob("*.md"))
    + list((root / ".azure").rglob("*.md"))
)
