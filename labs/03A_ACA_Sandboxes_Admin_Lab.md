...
- `python-sandbox/` Code Interpreter custom disk image 기반 Sandbox 생성
- Python SDK로 코드 실행
- 파일 쓰기·읽기·목록·삭제
...
검증했다. 결과는 `.work/sandboxes-live/python-validation.json`에 저장된다.
...
export LAB_WORK_DIR="$PWD/.work/sandboxes"
...
az acr build \
  ...
  --file python-sandbox/Dockerfile \
  python-sandbox/
```
...
RESOURCE_GROUP  = os.environ.get("RESOURCE_GROUP", "rg-ai-workspace-sandbox-lab")
...
Dynamic Sessions의 `Azure ContainerApps Session Executor`에 대응하는 역할이다.
...
# sandbox ID를 파일로 보관
with open(os.path.join(os.environ.get("LAB_WORK_DIR", ".work/sandboxes"), "sandbox_id.txt"), "w") as f:
...
> Dynamic Sessions와 달리 Sandbox는 개별 VM이므로 생성에 수 초~수십 초가 걸린다.  
...
LOCATION        = os.environ.get("LOCATION", "koreacentral")
SANDBOX_GROUP   = os.environ.get("SANDBOX_GROUP_NAME", "ai-workspace-sandboxes")
WORK_DIR        = os.environ.get("LAB_WORK_DIR", ".work/sandboxes")
...
Sandbox에 CSV 파일과 Python 분석 코드를 작성한 뒤 실행한다.  
Dynamic Sessions가 multipart `POST /files` REST API를 사용했다면, Sandboxes는 SDK `write_file()`을 사용한다.
...
Dynamic Sessions의 `EgressDisabled`와 동일한 결과를 다른 방식으로 구현한다.
...
Dynamic Sessions의 이진 차단과 달리 Sandboxes는 세분화 허용 규칙을 지원한다.  
...
> Dynamic Sessions는 세션마다 `/mnt/data`를 쓰고 세션 식별자로 구분한다.  
> Sandboxes는 같은 Sandbox ID를 사용하므로 상태가 그대로 유지된다.

## 16. Suspend와 Resume (일시 중단과 재개)

Dynamic Sessions는 cooldown 후 자동 삭제되지만, Sandboxes는 메모리·디스크 상태를 보존하고 재개할 수 있다.
...
Dynamic Sessions의 [실습 1A §16절](01A_Python_Code_Interpreter_Admin_Lab.md#16-llm-agent-backend-구성)과 동일하게 Azure OpenAI를 연결한다.  
Sandboxes용 gateway는 `python_gateway/` 대신 동일한 코드에서 `EXECUTION_BACKEND=sandboxes` 환경 변수로 전환한다.
...
> Sandboxes gateway 구현은 현재 `python_gateway/` 확장 항목이다.  
> Sandboxes backend는 세션 식별자 대신 Sandbox ID를 관리하며, 파일 I/O는 REST API 대신 SDK를 사용한다.
...
## 부록: Dynamic Sessions와 ACA Sandboxes 명령 대조

| 작업 | Dynamic Sessions | ACA Sandboxes |
| --- | --- | --- |
| 풀/그룹 생성 | `az containerapp sessionpool create` | `SandboxGroupManagementClient.create_group()` |
| 실행 환경 생성 | pool이 자동 할당 | `client.begin_create_sandbox().result()` |
| 인증 토큰 | `az account get-access-token --resource https://dynamicsessions.io` | `DefaultAzureCredential` (SDK 내장) |
| 코드 실행 | `POST /executions` curl | `sb.exec("python3 script.py")` |
| 파일 업로드 | `POST /files` multipart | `sb.write_file("/path", content)` |
| 파일 다운로드 | `GET /files/{name}/content` | `sb.read_file("/path")` → `bytes` |
| 파일 목록 | `GET /files` | `sb.list_files("/path")` |
| Egress 차단 | `--network-status EgressDisabled` | `EgressPolicy(default_action='Deny', traffic_inspection='Full')` |
| 세션/샌드박스 삭제 | `DELETE /session` | `sb.delete()` |
| 상태 유지 | 불가 (ephemeral) | `sb.begin_stop()` / `sb.begin_resume()` |
| Snapshot | 불가 | `sb.begin_create_snapshot().result()` |
| RBAC 역할 | `Azure ContainerApps Session Executor` | `Container Apps SandboxGroup Data Owner` |
| 데이터 평면 엔드포인트 | `<region>.dynamicsessions.io` | `management.<region>.azuredevcompute.io` |
