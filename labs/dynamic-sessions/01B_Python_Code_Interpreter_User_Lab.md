...
이 repository에서는 `dynamic_sessions.python_gateway`가 AI Workspace backend를 대리한다. Gateway만 LLM 설정, pool endpoint, Entra token과 session identifier를 관리하며 사용자 API에는 public analysis job ID만 반환한다.
...
python -m dynamic_sessions.python_gateway
```
...
```bash
mkdir -p .work/dynamic-sessions/python-user

cat > .work/dynamic-sessions/python-user/sales.csv <<'CSV'
...
curl --fail-with-body --silent --show-error \
  ...
  --form "file=@.work/dynamic-sessions/python-user/sales.csv;filename=sales.csv" \
  --form "expected=monthly_sales.png" \
  --form "expected=summary.json" \
  --output .work/dynamic-sessions/python-user/create.json
...
jq '{
  ...
  promotions
}' .work/dynamic-sessions/python-user/create.json

export ANALYSIS_JOB_ID=$(jq -r '.id' .work/dynamic-sessions/python-user/create.json)
```

통과 기준:

- `status`는 `completed`
- `classification`은 `A`, `route`는 `python`
- `succeeded`는 `true`
...
curl --fail-with-body --silent --show-error \
  ...
  --header "X-Demo-User: $DEMO_USER" \
  --output .work/dynamic-sessions/python-user/summary.json

jq . .work/dynamic-sessions/python-user/summary.json
```
...
curl --fail-with-body --silent --show-error \
  ...
  --data '{}' \
  --output .work/dynamic-sessions/python-user/approved.json
